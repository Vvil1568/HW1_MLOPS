...
class DVCManager:
    ...
    def _init_dvc(self):
        if not os.path.exists(os.path.join(self.data_dir, ".dvc")):
            ...
            cmd = [
                ...
                f"s3://{settings.S3_BUCKET_NAME}/dvc"
            ]
            ...
            subprocess.run(["dvc", "remote", "modify", "minio", "secret_access_key", settings.AWS_SECRET_ACCESS_KEY],
                           cwd=self.data_dir)
            # Disable SSL verify for internal Minio
    ...
    def add_and_push(self, filename: str):
    ...
    def list_files(self):
        # Scan for .dvc files
        files = [f.replace(".dvc", "") for f in os.listdir(self.data_dir) if f.endswith(".dvc")]
        return files